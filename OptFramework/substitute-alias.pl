...
# JEFF: the next two while loops used to run as a single loop. Your modifications 
# correctly repointed the aliased $gChoices{$attribute} to the right string name 
# when aliases were found, but the loop continued to use the original alias +-------+
# to process the associated conditions. I've broken it up into too loops;           |
# the second one reinitializes the choice variable using the string names      (defined here)
# you mapped.                                                                       |
#                                                                                   |
#                           +-------------------------------------------------------+ 
#                           |
...
while ( my ( $attribute, $choice) = each %gChoices ){
  ...
  if ( ! defined( $gOptions{$attribute} ) ){
	...
	debug_out ( "   - found attribute \$gOptions{\"$attribute\"} in options file \n"); 
  
  }
  
  # is choice in options ?
  debug_out ("   - (Does \$gOptions{$attribute}{\"options\"}{$choice} exist ? ");   
	...
	if ( $choice =~ /\d{3,4}/ ){
		...
		for my $optionIndex ( keys (%$OptHash ) ){
			if ( $gOptions{$attribute}{"options"}{$optionIndex}{"alias"} =~ /$choice/ ) {
				debug_out("   - Found valid alias $gChoices{$attribute}, pointing to $optionIndex\n");
                $gChoices{$attribute} = $optionIndex;	#"The Option name"
                debug_out("   - now: \$gChoices{$attribute} = $gChoices{$attribute}\n");  
				$allok = 1;
				last;
        ...
        # number is not a numerical alias. 
        # Do nothing --- Let substiture.pl report failure
    }
...
# Now the values in $gChoices have been re-mapped to use strings instead of aliases.  
# Process conditions using the strings that substitute.pl typically expects   
while ( my ( $attribute, $choice) = each %gChoices ){
